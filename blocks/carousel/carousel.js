...
function updateActiveSlide(slide, hasInnerIndicators) {
  ...
  const carouselSlideContents = document.querySelectorAll(
    ".carousel-slide-content"
  );
  carouselSlideContents.forEach((content) => {
    const hasContent =
      content.textContent.trim().length > 0 || content.children.length > 0;
    if (!hasContent) {
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting)
          updateActiveSlide(entry.target, hasInnerIndicators);
      });
    },
    { threshold: 0.5 }
  );
...
export default async function decorate(block) {
  ...
  const hasInnerIndicators = document.querySelector(
